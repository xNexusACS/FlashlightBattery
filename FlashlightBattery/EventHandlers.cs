...
using FlashlightBattery.Components;
using UnityEngine;
    ...
    public class EventHandlers
    {
            ...
            if (ev.NewItem?.Type == ItemType.Flashlight && heldFlashlights.ContainsKey(ev.NewItem.Base))
            {
                ...
                if (component != null)
                {
                    if (component.IsBatteryDead())
                    {
                        ...
                        ev.Player.ShowHint($"{new string('\n', MainClass.singleton.Config.FlashlightHintTextLower)}{MainClass.singleton.Config.FlashlightIsDead}", MainClass.singleton.Config.DeadFlashlightHintTime);
                    }
                    else
                    {
        ...
        private IEnumerator<float> ShowHint(Player player, BatteryComponent component)
        {
            while (true)
            {
                ...
                string bString = $"{new string('\n', MainClass.singleton.Config.FlashlightHintTextLower)}{MainClass.singleton.Config.FlashlightBatteryHint.Replace("{percent}", Mathf.Clamp(battery, 0f, 100f).ToString())}";
                if (battery <= 25)
                {
                    bString = bString.Insert(MainClass.singleton.Config.FlashlightHintTextLower, battery <= 10 ? "<color=red>" : "<color=yellow>");
                    bString += "</color>";
                }
                player.ShowHint(bString, 1f);
